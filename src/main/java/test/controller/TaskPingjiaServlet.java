 ...
 */
@WebServlet(name = "TaskPingjiaServlet",urlPatterns = {"/TaskPingjiaServlet"})
public class TaskPingjiaServlet extends HttpServlet {
...
//        try {
//            agentCenter.Adjust_agent("agentPingjiaTask",request,response,logger);
//        } catch (SQLException e) {
//            e.printStackTrace();
//        }
        try {
            Agent_baseFunction agent_baseFunction=new Agent_baseFunction();
            agent_baseFunction.agentPingjiaTask(request,response);//调用agent评价任务
          //  Agent1.agentPingjiaTask(request,response);//调用agent评价任务
        } catch (SQLException e) {
            e.printStackTrace();
        }
